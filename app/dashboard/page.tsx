      ...
      <section className="mx-auto w-full max-w-[1440px] px-5 py-9 sm:px-9">
        <div className="mb-8">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-[var(--muted)]">
            Miles. Fuel. Content. Keep showing up and log this month&apos;s content.
          </p>
        </div>
        ...
        <div className="mb-7 inline-flex items-center gap-2 rounded-md bg-[var(--yellow)] px-4 py-2 text-sm font-medium text-black">
          ...
          <span>| Ambassador content portal</span>
        </div>
        ...
        <section className="mb-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            ["Total uploads", checkIns.length, "content pieces saved", false],
            ["Last upload", getLastCheckInLabel(checkIns), "latest logged day", false],
            ...
            {checkIns.length ? (
              <div className="space-y-3">
                {checkIns.map((item) => (
                  <article
                    key={item.id}
                    className="grid items-start gap-3 rounded-md bg-[var(--black-3)] p-3 sm:grid-cols-[110px_1fr]"
                  >
                    <div className="self-start overflow-hidden rounded-md bg-[var(--black-4)]">
                      {item.signedPhotoUrl ? (
                        isVideoAsset(item.photo_url) ? (
                          ...
                          <img
                            src={item.signedPhotoUrl}
                            alt={`Upload from ${item.check_in_date}`}
                            className="block h-28 w-full object-cover"
                          />
                        )
                      ) : (
              ...
              <div className="rounded-md border border-dashed border-[#333] p-8 text-center text-sm text-[var(--muted)]">
                No uploads yet. Submit your monthly content to start the library.
              </div>
            )}
          </div>
        </section>
